...
# Keep this script's custom model directory name under repo data/
save_dir = NEWBOUNDS_TRAIN_DIR
model_dir = REPO_ROOT / "data" / "newbounds_8_samplestotrain_model_directory"
...
def get_confidence_interval_indicator_ghx(x_ghx_sim_experiment, valid_indices, plot_index):
    ...
    try:
        ...
        for i, sample in enumerate(full_data):
            ...
            if i % 1000 == 0:
                print(
                    f"t={i:4d} | mean={sample_mean:.3f}, MoE={margin_of_error:.3f}, "
                    f"CI=({lower:.3f}, {upper:.3f}), exp={experience[i]:.3f}, error={abs_error:.3f}"
                )
        ...
        print(
            f"CI Width @ Max Error: "
            f"{confidence_interval_indicator_max[np.argmax(error_indicator_max)]:.4f}"
        )
...
def plot_simulation_and_metrics(x_ghx_sim_experiment, selected_model_len):
    ...
    for row, plot_indices in enumerate([0, 1]):
        ...
        for p in valid_indices:
            ...
            rmse = np.sqrt(
                mean_squared_error(
                    x_ghx_experiment_conv[plot_indices, :],
        ...
        rmse_mean_sim = np.sqrt(
            mean_squared_error(x_ghx_experiment_conv[plot_indices, :], mean_sim)
        )
        ...
        ax[row][0].plot(
            savgol_filter(x_ghx_experiment[plot_indices, :], 1200, 3),
            '-',
            color='green',
            label='smoothed experiment'
        )
...
def SINDyC_fit(x_tes_train, u_tes_train, tm2, threshold=1e-3, alpha=1e-3, name="SinDyc", verbose=True):
    ...
    """
    poly_library = ps.PolynomialLibrary(degree=1, include_interaction=False)
    ...
    model_tes.fit(x=x_tes_train, u=u_tes_train, t=tm2[1] - tm2[0])

    with open(f"{name}.pkl", "wb") as outfile:
...
def process_model(t, x, u, model_tes, model_ghx):
    ...
    dTmid_tes_dt = (
        model_tes.coefficients()[3][0]
        + model_tes.coefficients()[3][1] * m_tes
        + model_tes.coefficients()[3][2] * Tout_tes
        + model_tes.coefficients()[3][3] * Ttop_tes
        + model_tes.coefficients()[3][4] * Tmid_tes
        + model_tes.coefficients()[3][5] * Tbot_tes
    ...
    dTbot_tes_dt = (
        model_tes.coefficients()[4][0]
        + model_tes.coefficients()[4][1] * m_tes
        + model_tes.coefficients()[4][2] * Tout_tes
        + model_tes.coefficients()[4][3] * Ttop_tes
        + model_tes.coefficients()[4][4] * Tmid_tes
        + model_tes.coefficients()[4][5] * Tbot_tes
...
def simulate_sindyc(model_tes, model_ghx, x_tes, x_ghx, u_ghx_train, t_hat):
    ...
    for i in range(0, len(t_hat) - 2):
        ...
        sol = solve_ivp(
            ...
            t_hat[i:i + 2],
            y_hat[i],
            args=(u_hat, model_tes, model_ghx),
            **integrator_keywords
        )
...
def simulate(model_tes, model_ghx, mean, covariance, mean_ghx, covariance_ghx):
    """
    Integrate the SINDyC model from experimental initial conditions
    """
    model_tes.model[-1].coef_ = np.random.multivariate_normal(
        mean=mean,
        cov=covariance
    ).reshape(model_tes.model[-1].coef_.shape[0], model_tes.model[-1].coef_.shape[1])

    model_ghx.model[-1].coef_ = np.random.multivariate_normal(
        mean=mean_ghx,
        cov=covariance_ghx
    ).reshape(model_ghx.model[-1].coef_.shape[0], model_ghx.model[-1].coef_.shape[1])

    results = simulate_sindyc(
        model_tes,
        model_ghx,
        x_tes_experiment_conv.T,
        x_ghx_experiment_conv.T,
        u_experiment_conv_2.T,
        tm2
    )
    return results
...
x_tes_experiment_conv = finer(
    np.linspace(0.0, tm2[-1], x_tes_experiment_smooth.shape[1]),
    x_tes_experiment_smooth,
    tm2[:-1]
)
x_ghx_experiment_conv = finer(
    np.linspace(0.0, tm2[-1], x_ghx_experiment_smooth.shape[1]),
    x_ghx_experiment_smooth,
    tm2[:-1]
)
u_experiment_conv = finer(
    np.linspace(0.0, tm2[-1], u_experiment.shape[1]),
    u_experiment,
    tm2[:]
)
...
model_tes_exp = SINDyC_fit(
    x_tes_experiment_new,
    u_experiment_new,
    tm2,
    threshold=threshold,
    alpha=alpha,
...
model_ghx_exp = SINDyC_fit(
    x_ghx_experiment_new,
    u_experiment_new,
    tm2,
    threshold=threshold,
    alpha=alpha,
...
for incr, elem in enumerate(final_set):
    ...
    threshold = 1e-6
    alpha = 1e-3
    model_tes = SINDyC_fit(
        list(x_tes_train),
        list(u_tes_train),
        tm2,
        threshold=threshold,
        alpha=alpha,
        name=str(model_dir / f"TES_{incr+1}"),
        verbose=False
    )
    ...
    threshold = 1e-8
    alpha = 1e-6
    model_ghx = SINDyC_fit(
        list(x_ghx_train),
        list(u_ghx_train[:, :, :-1]),
        tm2,
        threshold=threshold,
        alpha=alpha,
        name=str(model_dir / f"GHX_{incr+1}"),
        verbose=False
    )
...
with open(model_dir / "TES_UQ_all_coefficients.pkl", "wb") as f:
    pickle.dump(model_para_tes, f)

with open(model_dir / "GHX_UQ_all_coefficients.pkl", "wb") as f:
    pickle.dump(model_para_ghx, f)

with open(model_dir / "EXP_UQ_all_coefficients.pkl", "wb") as f:
...
selected_models = random.sample(all_indices, initial_count)
...
while len(selected_models) <= total_models:
    ...
    print(f"[Iteration {iteration}] Selected Models: {len(selected_models)}")
    ...
    valid_ghx, avg_rmse0, avg_mae0, avg_rmse1, avg_mae1, rmse0_mean, mae0_mean, rmse1_mean, mae1_mean = plot_simulation_and_metrics(
        x_ghx_sim_experiment,
        selected_model_len=len(selected_models)
    )
    ...
    cumulative_runtime = time.perf_counter() - t0
    ...
    results.append({
        ...
        'cumulative_runtime_sec': float(cumulative_runtime),
    })

    if len(selected_models) == total_models:
        break

    next_models = random.sample(remaining_indices, min(increment, len(remaining_indices)))
    selected_models.extend(next_models)
    remaining_indices = list(set(remaining_indices) - set(next_models))
    iteration += 1

print(f"Total selected models: {len(selected_models)} (Expected: {total_models})")

results_df = pd.DataFrame(results)
results_csv = RESULTS_CSV_DIR / f"time4rndm_{numb_trajectories}traj_{initial_count}init{increment}_CI_rmse_mae_results.csv"
