...
numb_trajectories = 12 # number of trajectories to fit each model 12
...
import time  # <-- add this near your other imports

iteration = 0
results = []

t0 = time.perf_counter()  # global timer start

while len(selected_models) <= total_models:
    iter_start = time.perf_counter()  # iteration timer start
    ...
    if ncores > 1:
        ...
        # (optional) serial fallback if you want it
        fitness = [simulate(
            model_tes_all[random.choice(selected_models)],
            model_ghx_all[random.choice(selected_models)],
            mean, covariance, mean_ghx, covariance_ghx
    ...
    results.append({
        ...
        'iter_runtime_sec': float(iter_runtime),              # NEW
        'cumulative_runtime_sec': float(cumulative_runtime),  # NEW
    })
...
# Save all results (fix f-string)
results_df = pd.DataFrame(results)
results_df.to_csv(
    f"time4rndm_{numb_trajectories}traj_{initial_count}init{increment}_CI_rmse_mae_results.csv",
    index=False
)
