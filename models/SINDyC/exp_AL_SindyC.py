...
rand_numb   = 500         # MC draws for MVG-SINDy experiment eval (keep your global)
ncores      = 100            # parallelism for scoring and MC
OUT_METRICS = "incExp_errorAL10sindy_exp_metrics.csv"
...
while True:
    ...
    # 4) Score remaining with pooled models and pick worst k
    if len(remaining_idx) > 0:
        print("  - Scoring remaining trajectories...")
        if ncores > 1:
            with joblib.Parallel(n_jobs=ncores) as parallel:
                scored = parallel(joblib.delayed(_score_one)(idx, model_tes_cv, model_ghx_cv)
                                  for idx in remaining_idx)
        else:
            scored = [_score_one(idx, model_tes_cv, model_ghx_cv) for idx in remaining_idx]

        scored.sort(key=lambda t: t[1], reverse=True)  # worst first
        k = min(INCREMENT, len(scored))
        new_pick = [idx for (idx, _, _, _) in scored[:k]]
        print(f"  - Adding worst {k} indices: {new_pick}")
        selected_idx.extend(new_pick)
        remaining_idx = [i for i in remaining_idx if i not in new_pick]

        if EXP_INDEX in new_pick and exp_included_iter is None:
            exp_included_iter = iteration
            print(f"[AL] NOTE: EXP (index {EXP_INDEX}) entered training set at Iter {iteration}")
    else:
        print("  - No remaining trajectories to score.")
        break
