...
# Config
# -----------------------------
DATA_DIR  = "ghx_data_csv"   # ghx_run{ID}.csv files
...
HIDDEN   = 256
BATCH    = 512
EPOCHS   = 8  # retrains many times
LR       = 1e-3
NUM_WORKERS = 4
CLIP_Z   = 3.5

CTRL_NAMES  = ["opening_PV006","mflow_pump_out","T_pump_in","T_heater_out"]   # 4
STATE_NAMES = ["mflow_GHX_bypass","qghx_kW"]                                   # 2

OUT_DIR  = "time_rndm_gru_sampling_results"
...
def plot_timeseries(iteration, n_files, t, m_true, m_pred, q_true, q_pred):
    ...
    ax1.set_xlabel("time (s)"); ax1.set_ylabel("mflow_GHX_bypass")
    ...
    ax2.set_xlabel("time (s)"); ax2.set_ylabel("qghx_kW")
    ax2.set_title(f"Iter {iteration} (n={n_files}) q vs time")
    ax2.grid(True, alpha=0.3); ax2.legend()

    plt.tight_layout()
    out = os.path.join(PLOT_DIR, f"iter_{iteration:03d}_n{n_files}_timeseries.png")
...
while len(selected_files) <= total_files:
    ...
    # dataset + model (train from scratch for fair comparison)
    ...
    results.append({
        ...
        "rmse_m": metrics["rmse_m"], "mae_m": metrics["mae_m"], "r2_m": metrics["r2_m"],
        "rmse_q": metrics["rmse_q"], "mae_q": metrics["mae_q"], "r2_q": metrics["r2_q"],
